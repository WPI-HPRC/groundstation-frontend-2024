...
void HPRCStyle::drawHPRCPayloadServo(QPainter *p, const hprcDisplayWidget *w)
{
    p->setRenderHint(QPainter::Antialiasing);

    QPen textPenRed = QPen(m_highlightBrush, 3);
    QPen textPenWhite = QPen(m_textBrush, 3);
    QPen rectPen = QPen(m_textBrush, 10); //Width of rectangles depends on height
    rectPen.setCapStyle(Qt::RoundCap);
    QPen rectLightPen = QPen(m_highlightBrush, 10);
    rectLightPen.setCapStyle(Qt::RoundCap);

    p->setPen(rectPen);
    QPointF rectStart = QPointF(w->width() / 2, w->height() * (3/4));
    float rectWidth = w->width() / 10;
    float rectHeight = w->height() / 3;

    QRect rect = QRect(rectStart.x() - rectWidth / 2, rectStart.y(), rectWidth, rectHeight);

    p->fillRect(rect, m_highlightBrush);
